...
*/
	...
	boolean[] readFormatInformation() {
		...
		for (int i = 0; i <= 5; i++)
			modules[i] = moduleMatrix[8][i];
		
		modules[6] = moduleMatrix[8][7];
		modules[7] = moduleMatrix[8][8];
		modules[8] = moduleMatrix[7][8];
		
		for (int i = 9; i <= 14; i++)
			modules[i] = moduleMatrix[14-i][8];
	...
	public int[] getBlocks() {
		...
		do {
			...
			codeBits.addElement(new Boolean(moduleMatrix[x][y]));
	...
	//OKK
	private void reverseElement(int x, int y) {
