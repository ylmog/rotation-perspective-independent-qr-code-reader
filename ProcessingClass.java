...
//	static boolean performance=true;//true false
	boolean useTwoUncor=true; //true false
	private static final int reduc=1;
	
	//private long startTime;
	//final static boolean debugStats = true;
	...
	public String decodeBin(Image acilan){ // String path, String folder
     ...
     //   if(performance) this.ImageWindow(folder+"1gray.jpg",ima);
        ...
        //if(performance){new ImageWindow("2 - Resized Image",ima,false);}
        //adaphisteq(ima,20);
        ...
        //if(performance){new ImageWindow("3 - Binarized Image",ima,true);}
        double[][] coor=new double[3][4];
        int distance=finderPattern(ima,coor);
        double cell=cellSize(coor);
        //if(performance)System.out.println("Cell size : " + cell);
        //if(performance)System.out.println("Distance : " + distance);

        ima = imTwoDTransform(ima,coor,cell,distance);
        //if(performance){new ImageWindow("4 - Transformed Image",ima,true);}
        
        useMyfilter(ima,coor,4,cell);
        //if(performance){new ImageWindow("4.5 - Filtered Transformed Image",ima,true);}
        
        double[][] corners = findCorners(ima,coor,(int)scaRound(cell),4);
        //if(performance)System.out.println("4 Corners");
        //if(performance)dispCorners(corners);
        ...
        //if(performance){new ImageWindow("5 - Perspective Transformed Image",ima,true);}
        useMyfilter(ima,coor,3,cell);
        //if(performance){new ImageWindow("5.5 - Filtered Perspective Transformed Image",ima,true);}
        ...
        //if(performance){new ImageWindow("6 - Croppped Image",ima,bounds);}
        ...
        //if(performance){new ImageWindow("7 - Binarized Image",image);}
	...
	private boolean[][] toBinary2(int[][] image,int[][] bounds,int bits){
		...
		double oranx=(double)col/bits;
		double orany=(double)row/bits;
		//if(performance) System.out.println("oranx "+ oranx+" orany : "+orany);
		int tb,et;
		//int tbb,etb;
		...
		for(row=0;row<bits;row++){
	    	for(col=0;col<bits;col++){
	    		...
	    		//if( performance)if((row>=13 && col>=8) && (row<=20 && col<=20))System.out.println("Now at point - x : "+et+" y : "+tb+" for x : "+col+" y : "+row);
	    		...
	    		if(say>=3 && image[tb][et]==1){
	    			...
	    			//if(performance)System.out.println("Decide : "+sum(image,tb-1,tb+1,et-1,et+1));
	    		}
	...
	private int numberofCells2(int[][] image,double[][] coord,double cell) {
		...
		//if(performance)System.out.println("Cell counting at x : "+xpos +" ypos : "+ypos);
		...
		int num=inc+13;
		//if(performance)System.out.println("Number of cell1 : "+num);
		//% % Double Check % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
		inc=0;
		xpos=coorint[0][0];
		ypos=coorint[0][1];
		...
		inc+=13;
		//if(performance)System.out.println("Number of cell1 : "+inc);
		if(inc!=num){
	...
	private double[][] findCorners(int[][] image,double[][] coor,int cell,int kackose){
		...
		corners[0][0]=xpos;			
		corners[0][1]=ypos;
		//if(performance)System.out.println("xpos : "+xpos +" ypos : "+ypos);
		//if(performance)System.out.println("start : "+(ypos-1) +" stop : "+(ypos-2*cell));
		if(kackose==3){
		...
		//if(performance)System.out.println("4 Corners (First 3 one)");
		//if(performance)dispCorners(corners);
		//if(performance)System.out.println("Alignment");
		//if(performance)dispCorners(alignment);
		//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		if(kackose==3){
		...
		for(h=(int)alignment[2][0]+step;h<boundry;h+=step){
			...
			//if( performance)System.out.println("Progress x : "+progss[2][0]+" y :"+progss[2][1]);
			...
			if(found){
				...
				//if( performance)System.out.println("End Point x : "+endpoint[2][0]+" y :"+endpoint[2][1]);
			}
			found=false;
		}
		...
		//if(performance)System.out.println("New Alignments ");
		//if(performance)dispCorners(alignment);
		...
		//if(performance)System.out.println("All 4 corners !");
		//if(performance)dispCorners(corners);
	...
	private int finderPattern(int[][] gg,double[][] coor){
		...
		//if(performance)System.out.println("All patterns");
		//if(performance)disp(coordh);
		if(coordh.size()<3)
		...
		//if(performance)System.out.println("3 patterns");
		//if(performance)disp(coordh);
		...
		//if(performance)System.out.println("3 patterns - middle corrected");
		//if(performance)disp(coordh);
		...
		c[2][1]*=-1;		
		c[1][1]*=-1;
		//if(performance)System.out.println("Angle Coordinates.");
		//if(performance)for(i=1;i<3;i++){for(k=0;k<2;k++){System.out.print(c[i][k]+" ");}System.out.println();}
		
		//if(performance)System.out.println("Angles Seperately "+ Math.atan2(c[1][1],c[1][0])+" " +Math.atan2(c[2][1],c[2][0]));
		double angleBet=aTan2(c[1][1],c[1][0])-aTan2(c[2][1],c[2][0]);
		//if(performance)System.out.println("Angle between : "+ angleBet);
		if(c[1][1]<0){
			angleBet+=2*Math.PI;
		}
		if(c[2][1]<0){
		...
		angleBet%=2*Math.PI;
		//if(performance)System.out.println("Angle between : "+ angleBet);
		//if(performance)if(angleBet>2*Math.PI || angleBet<0)System.out.println("HATATATTATATA");
		...
		//if(performance)System.out.println("Corrected patterns");
		//if(performance)disp(coordh);
		for(i=0;i<3;i++)
			for(k=0;k<4;k++)
	...
	private int[][] imPerspecTransform5(int[][] image,double[][] corners,double[][] coor){
		...
		//if(performance)System.out.println("4 input points for 3D transform");
		//if(performance)for(i=0;i<4;i++){System.out.println("x : "+corners[i][0]+" y : "+corners[i][1]);}
		//if(performance)System.out.println("distance : "+dist);
		...
		//if(performance)System.out.println("Matrix and B - 3");
		//if(performance)for(i=0;i<8;i++){for(j=0;j<8;j++){System.out.print(matrx[i][j]+" ");}System.out.println();}
		//if(performance)for(i=0;i<8;i++){System.out.println(b[i]);}
		//if(performance)System.out.println("End of Matrix and B");
		...
		try {
			...
			b[4]+=dist;
			b[3]+=dist;
			//if(performance)System.out.println("Matrix and B - 2");
			//if(performance)for(i=0;i<8;i++){for(j=0;j<8;j++){System.out.print(matrx[i][j]+" ");}System.out.println();}
			//if(performance)for(i=0;i<8;i++){System.out.println(b[i]);}
			//if(performance)System.out.println("End of Matrix and B");
			soln=solve(matrx,b);
		...
		//if(performance)System.out.println("3D transformed centers");
		//if(performance)disp(coor);
		...
		//if(performance)System.out.println("Matrix to be inverted for 3D transform");
		//if(performance)for(i=0;i<3;i++){for(j=0;j<3;j++){System.out.print(matrx2[i][j]+" ");}System.out.println();}
		invmatrx=inverse(invmatrx);
		//if(performance)System.out.println("inverted for 3D transform");
		//if(performance)for(i=0;i<3;i++){for(j=0;j<3;j++){System.out.print(invmatrx[i][j]+" ");}System.out.println();}
		...
		for(i=0;i<size;i++){
			for(j=0;j<size;j++){
				...
				////if(performance)System.out.println(j+" "+i+" <- "+x+" "+y);
				if(x<0 || y<0 || x>=size || y>=size) {
					//if(performance)System.out.println("Skipped : i(y) : "+i+" j(x) : "+j);
					//throw new IllegalArgumentException("Skipped item!");
					retima[i][j]=1;//make it white
					continue;
				}
				retima[i][j]=image[y][x];
				//if(performance)System.out.println("Point : x : "+x+" , y : "+y);
			}
		}
		return retima;
	}
	...
	private double[][] inverse(double[][] mat){
		...
		//if(performance)System.out.println("Ara i�lemler");
		//if(performance)for(i=0;i<size;i++){for(j=0;j<size;j++){System.out.printf(" "+mat[i][j]);}System.out.printf("\n");}
		//if(performance)System.out.print("--------\n");
		//if(performance)for(i=0;i<size;i++){for(j=0;j<size;j++){System.out.print(" "+inv[i][j]);}System.out.println();}
		...
		//if(performance)System.out.println("Inverse");
		//if(performance)for(i=0;i<size;i++){for(j=0;j<size;j++){System.out.print(" "+inv[i][j]);}System.out.println();}
		return inv;
	}
	...
	private int[][] imTwoDTransform(int[][] image,double[][] coor,double cell,double distance) {	
		...
		//if(performance)System.out.println("Input Coordinates");
		//if(performance)disp(coor);
		//if(performance)System.out.println("Output Coordinates");
		//if(performance)for(i=0;i<3;i++){System.out.println("x : "+matrx1[i][0]+" y : "+matrx1[i][1]);}
		...
		//if(performance)System.out.println("Input Matrix");
		//if(performance)for(i=0;i<3;i++){for(j=0;j<3;j++){System.out.print(matrx1[i][j]+" ");}System.out.println();}
		//if(performance)for(i=0;i<3;i++){System.out.println(inp1[i]);}
		//if(performance)for(i=0;i<3;i++){System.out.println(inp2[i]);}
		...
		//if(performance)System.out.println("Translated centers");
		//if(performance)disp(coor);
	...
	private double[] solve(double[][] mat,double[] vector){
		...
		for(k=0;k<size;k++){
			...
			if(p!=k){
				...
				//Integer[] dumm={p,k};
				//changes.add(dumm);
				//if(performance)System.out.println("Change existed!!!!!");
			}
			//if(performance)System.out.println(k);
			if(mat[k][k]==0.0)
		...
		//if(performance)System.out.println("Solution");
		//if(performance)for(i=0;i<size;i++)System.out.println(" "+soln[i]);
		...
		//if(performance)System.out.println("Corrected Solution");
		//if(performance)for(i=0;i<size;i++)System.out.println(" "+soln[i]);*/
	...
	/*private int[][] imresize(int[][] image,double scale) throws NoninvertibleTransformException {
		...
		for(pointS.y=0;pointS.y<row;pointS.y++){
			for(pointS.x=0;pointS.x<col;pointS.x++){
				...
				//if(performance)System.out.println(pointD.getX()+" "+pointD.getY());
			}
		}
		return nima;
	}*/
	private void thresholdOtsu(int[][] image){
		...
		//for(int y=0;y<256;y++)
		//if(performance)System .out.println(counts[y]);
		...
		for(i=0;i<counts.length;i++){
			...
			//if(performance)System.out.println(counts[i]);
		}
	...
	public int[][] createImageMatrix(Image acilan){
        ...
        for(y=0;y<ima.length;y++){
      	  for(x=0;x<ima[0].length;x++){
		      ...
		      //if(performance)System.out.print(ima[y][x]+" ");
      	  }
	      //if(performance)System.out.println();
        }
        return ima;
	}
	...
	public Image openImage(String file){
		...
		while(acilan.getHeight(null)<0 || acilan.getWidth(null)<0 ){
			try {
				int time=4000/reduc;
				//if(performance)System.out.println("Sleeping for "+ time +"ms");
				Thread.sleep(time);
