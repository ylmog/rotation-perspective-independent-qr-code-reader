...
public class MatrixToString {
	 ...
	 * 	public static void main(String[] args) {
		...
		boolean[][] image = null;
		try {
			okudosya = new FileReader("res_data.txt");
			oku = new BufferedReader(okudosya);
			String line = oku.readLine();
			size = (line.length() + 1) / 2;
			image = new boolean[size][size];
			for (int q = 0; q < size - 1; q++) {
				for (int w = 0; w < size; w++) {
					image[q][w] = line.charAt(2 * w) == '0';
					// System.out.print(line.charAt(2*w)+" ");
				}
				line = oku.readLine();
				// System.out.println();
			}
		...
		System.out.println(inst.decode());
	}*/
